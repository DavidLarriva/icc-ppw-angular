  ...
  static getTextError(errors: ValidationErrors): string | null {
    for (const key of Object.keys(errors)) {
      switch (key) {
        ...
        case 'requiredTrue': return 'Debe aceptar este campo'; // <-- NUEVO CASE
        case 'minlength': return `Mínimo ${errors['minlength'].requiredLength} caracteres`;
        case 'maxlength': return `Maximum ${errors['maxlength'].requiredLength} caracteres`;
        case 'min': return `Debe ser mayor a: ${errors['min'].min}`;
        case 'max': return `Valor máximo: ${errors['max'].max}`;
